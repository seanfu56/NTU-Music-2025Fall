...
def softmax_with_temperature(logits, temperature):
    logits = logits.astype(np.float64)          # 保險一點用 float64
    logits = logits / max(temperature, 1e-6)    # 避免除到太小的溫度

    # numerically stable: 減掉最大值避免 exp 爆掉
...
class TransformerModel(nn.Module):
    ...
    def froward_output_sampling(self, h, y_type, temperature, top_p):
        '''
        for inference
        '''
        # sample type
        ...
        cur_word_barbeat =  sampling(y_barbeat, t=temperature)
        cur_word_chord =    sampling(y_chord, p=top_p)
        cur_word_pitch =    sampling(y_pitch, p=top_p)
    ...
    def inference_from_scratch(self, dictionary, temperature=1.0, top_p=0.9):
        ...
        with torch.no_grad():
            ...
            while(True):
                # sample others
                next_arr = self.froward_output_sampling(h, y_type, temperature, top_p)
...
def generate(cond, cond_file, num_songs, path_gendir, temperature, top_p):
    # path
    path_ckpt = info_load_model[0] # path to ckpt dir
    loss = info_load_model[1] # loss
    name = 'loss_' + str(loss)
    # path_saved_ckpt = os.path.join(path_ckpt, name + '_params.pt'
    path_saved_ckpt = 'ckpt/2_cpword.pt'
    ...
    while sidx < num_songs:
        try:
            ...
            if cond:
                res = net.inference_condition(dictionary, cond_file, temperature, top_p)
            else:
