...
class MambaModel(nn.Module):
    ...
    def inference_from_scratch(self, dictionary):
        """
        這裡保留你原本的流程，只是底層 encoder 換成 Mamba。
        為了簡單，我沒有做 streaming cache，而是每一步只餵 (B=1, L=1) 進 Mamba，
        其實就是「真．一步一步」的 recurrent 生成。
        """
        ...
        with torch.no_grad():
            ...
            for step in range(init.shape[0]):
                ...
                input_ = init_t[step, :].unsqueeze(0).unsqueeze(0)  # (1,1,7)
